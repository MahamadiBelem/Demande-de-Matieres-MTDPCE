...
/**
 * REST controller for managing {@link demande.matieres.domain.Structure}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Structure> partialUpdateStructure(
        ...
        Optional<Structure> result = structureRepository
            ...
            .map(existingStructure -> {
                if (structure.getLibelleStructure() != null) {
                    existingStructure.setLibelleStructure(structure.getLibelleStructure());
                }
                if (structure.getCodeStructure() != null) {
                    existingStructure.setCodeStructure(structure.getCodeStructure());
                }
